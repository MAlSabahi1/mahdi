...
export const DataGrid = forwardRef(function DataGrid<T>(
  ...
  const extractColumns = useCallback((defs: any[]): ColumnConfig[] => {
    ...
    defs.forEach(def => {
      if (def.children) {
        ...
        let label = def.headerName || def.field;
        if (def.field === "rowNumber") label = "الرقم التسلسلي (م)";
        if (def.field === "selection") label = "مربعات التحديد (Select)";
        
        let defaultVisible = true;
        if (def.field === "rowNumber" && showRowNumbers === false) defaultVisible = false;
        if (def.field === "selection" && showSelectionCheckboxes === false) defaultVisible = false;
        
        cols.push({
          field: def.field,
          headerName: label,
          isVisible: defaultVisible,
          isExportable: def.field !== "selection" && def.field !== "rowNumber" && def.field !== "actions",
          isEditable: def.editable !== false && def.field !== "rowNumber" && def.field !== "selection",
        });
      }
    });
    return cols;
  }, [showRowNumbers, showSelectionCheckboxes]);
  ...
  const isRowNumbersVisible = useMemo(() => {
    const config = columnConfigs.find(c => c.field === "rowNumber");
    return config ? config.isVisible : showRowNumbers;
  }, [columnConfigs, showRowNumbers]);

  const isSelectionVisible = useMemo(() => {
    const config = columnConfigs.find(c => c.field === "selection");
    return config ? config.isVisible : showSelectionCheckboxes;
  }, [columnConfigs, showSelectionCheckboxes]);

  const toggleRowNumbers = useCallback(() => {
    setColumnConfigs(prev => {
      const exists = prev.some(c => c.field === "rowNumber");
      if (!exists) {
        const allCols = extractColumns(columnDefs);
        return allCols.map(c => c.field === "rowNumber" ? { ...c, isVisible: !showRowNumbers } : c);
      }
      return prev.map(c => c.field === "rowNumber" ? { ...c, isVisible: !c.isVisible } : c);
    });
  }, [columnDefs, extractColumns, showRowNumbers]);

  const toggleSelectionCheckboxes = useCallback(() => {
    setColumnConfigs(prev => {
      const exists = prev.some(c => c.field === "selection");
      if (!exists) {
        const allCols = extractColumns(columnDefs);
        return allCols.map(c => c.field === "selection" ? { ...c, isVisible: !showSelectionCheckboxes } : c);
      }
      return prev.map(c => c.field === "selection" ? { ...c, isVisible: !c.isVisible } : c);
    });
  }, [columnDefs, extractColumns, showSelectionCheckboxes]);
  ...
  const processedColumnDefs = useMemo(() => {
    const processDefs = (defs: any[]): any[] => {
      return defs.map(def => {
        ...
        // Hide directly if disabled by props
        if (def.field === "rowNumber" && showRowNumbers === false) {
          isVisible = false;
        }
        if (def.field === "selection" && showSelectionCheckboxes === false) {
          isVisible = false;
        }
        ...
        return {
          ...def,
          hide: !isVisible,
          suppressColumnsToolPanel: !isVisible,
          editable: config ? (config.isEditable && (currentUserRole === "admin" || currentUserRole === "hr")) : def.editable,
          pinned: config ? (config.pinned || def.pinned) : def.pinned,
        };
      });
    };
    ...
    return processed;
  }, [columnDefs, columnConfigs, currentUserRole, renderRowActions, rtl, isLoading, showRowNumbers, showSelectionCheckboxes]);
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button 
                  ...
                  className={isRowNumbersVisible ? "text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700" : "text-muted-foreground opacity-50 hover:bg-slate-100"}
                >
                  <ListOrdered className="h-5 w-5" />
                </Button>
              </TooltipTrigger>
              <TooltipContent>{isRowNumbersVisible ? "إخفاء الأرقام التسلسلية" : "إظهار الأرقام التسلسلية"}</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button 
                  ...
                  className={isSelectionVisible ? "text-cyan-600 hover:bg-cyan-50 hover:text-cyan-700" : "text-muted-foreground opacity-50 hover:bg-slate-100"}
                >
                  <CheckSquare className="h-5 w-5" />
                </Button>
              </TooltipTrigger>
              <TooltipContent>{isSelectionVisible ? "إخفاء مربعات التحديد" : "إظهار مربعات التحديد"}</TooltipContent>
            </Tooltip>
