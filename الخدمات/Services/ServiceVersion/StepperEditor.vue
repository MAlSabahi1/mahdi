<template>
  <v-card>
    ...
    <v-card-text>
      ...
      <div v-else class="text-center py-12">
        ...
        <h3 class="text-h6 mb-2 text-grey-darken-1">لا توجد تابات مضافة</h3>
        <p class="text-body-2 text-grey mb-4">
          ابدأ بإضافة تاب جديد لبناء النموذج الخاص بك
        </p>
        <v-btn
          @click="addNewStep"
          color="primary"
  ...
  <v-dialog v-model="add_step_dialog" width="400">
    <v-card>
      <v-card-title class="my-2"> بيانات التاب </v-card-title>
      <v-card-text>
        <v-form @submit.prevent ref="form">
          <v-text-field
            v-model="step_temp.title"
            label="ادخل عنوان التاب"
            :rules="[$required]"
            density="compact"
            variant="outlined"
            color="primary"
          ...
          <v-text-field
            ...
            label="ادخل العنوان الفرعي للتاب"
            density="compact"
            variant="outlined"
            color="primary"
          >
