...
from .models import PersonnelMaster, SuggestedCorrection, HistoricalMonthlyVariables, RankSettlement, PersonnelEvent
...
class PersonnelDetailSerializer(serializers.ModelSerializer):
    ...
    def get_recent_events(self, obj):
        events = PersonnelEvent.objects.filter(personnel=obj).order_by('-event_date')[:24]
        return PersonnelEventSerializer(events, many=True).data
