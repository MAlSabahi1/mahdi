    ...
    <div class="p-5 mb-6 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
      <div class="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <div class="flex flex-col items-center w-full gap-6 xl:flex-row">
          <div
            class="w-20 h-20 overflow-hidden border border-gray-200 rounded-full dark:border-gray-800 flex items-center justify-center bg-gray-100 dark:bg-gray-800"
          >
            <img
              :src="authStore.user?.profile_picture || '/images/user/owner.jpg'"
              alt="user"
          ...
          <div class="order-3 xl:order-2">
            ...
            >
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ authStore.isAdmin ? 'مدير نظام' : 'مستخدم عادي' }}</p>
              <div class="hidden h-3.5 w-px bg-gray-300 dark:bg-gray-700 xl:block" v-if="authStore.user?.country"></div>
              <p class="text-sm text-gray-500 dark:text-gray-400" v-if="authStore.user?.country">{{ authStore.user?.country }}</p>
            </div>
          </div>
          <div class="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
            <a
              v-if="authStore.user?.facebook_link"
              :href="authStore.user.facebook_link"
              target="_blank"
              rel="noopener"
              class="social-button"
            >
            ...
            <a
              v-if="authStore.user?.x_link"
              :href="authStore.user.x_link"
              target="_blank"
              rel="noopener"
              class="social-button"
            >
            ...
            <a
              v-if="authStore.user?.linkedin_link"
              :href="authStore.user.linkedin_link"
              target="_blank"
              rel="noopener"
              class="social-button"
            >
            ...
            <a
              v-if="authStore.user?.instagram_link"
              :href="authStore.user.instagram_link"
              target="_blank"
              rel="noopener"
              class="social-button"
            >
