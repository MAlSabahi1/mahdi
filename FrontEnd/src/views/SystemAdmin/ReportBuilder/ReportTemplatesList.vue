  ...
  <div>
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">قوالب التقارير المخصصة</h1>
        <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">
          إدارة وتصميم الترويسة والتذييل للتقارير.
        </p>
      </div>
      <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
        <router-link
          to="/admin/reports/templates/builder"
    ...
    <div class="mt-8 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm">
      ...
      <div v-if="loading" class="p-12 flex flex-col items-center justify-center">
        ...
        <p class="text-gray-500 dark:text-gray-400 font-bold">جاري تحميل القوالب...</p>
      </div>
      ...
      <DataTable
        ...
        :columns="columns"
        :data="templates"
        :search-placeholder="'ابحث عن قالب بالاسم أو الرمز...'"
      >
        ...
        <template #cell-actions="{ row }">
          <div class="flex items-center gap-2">
            <router-link :to="`/admin/reports/templates/builder/${row.id}`" class="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded-lg transition-colors cursor-pointer" title="تعديل">
              <FileEdit class="w-4 h-4" />
            </router-link>
            <button @click="deleteTemplate(row.id)" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors cursor-pointer disabled:opacity-30 disabled:hover:bg-transparent" title="حذف">
...
import { FileText, FileEdit, Trash2, Loader2 } from 'lucide-vue-next'
...
const columns = ref([
  { key: 'name', label: 'اسم القالب', sortable: true },
...
const fetchTemplates = async () => {
  ...
  try {
    ...
    Swal.fire('خطأ', 'حدث خطأ أثناء جلب القوالب', 'error')
  } finally {
    loading.value = false
  }
}
