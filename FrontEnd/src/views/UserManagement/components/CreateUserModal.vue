  ...
  <div class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/50 p-4" @click.self="$emit('close')">
    <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900">
      <div class="mb-5 flex items-center justify-between">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-white/90">إضافة مستخدم جديد</h2>
      ...
      <form @submit.prevent="handleCreate" class="space-y-4">
        <div>
          ...
          <input v-model="form.username" v-field-error="'username'" type="text" required
        ...
        <div>
          ...
          <input v-model="form.password" v-field-error="'password'" type="password" required minlength="8"
            class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
...
async function handleCreate() {
  ...
  try {
    ...
    error.value = err.response?.data?.error || err.response?.data?.detail || 'فشل إنشاء المستخدم'
  } finally {
    loading.value = false
  }
}
