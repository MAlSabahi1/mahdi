  ...
  <admin-layout>
    <div class="space-y-6 pb-20">
      <div class="flex flex-wrap items-center justify-between gap-4 print:hidden">
        <div>
          <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">كشف الواصلين من الوزارة</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">نموذج رقم (١٨)</p>
        </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 overflow-hidden printable-area print:overflow-visible print:border-none print:shadow-none print:bg-transparent">
        <div class="p-6 print:p-0">
          ...
          <ReportTable 
            ...
            emptyDescription="لا يوجد واصلين مسجلين."
          ...
          >
            <template #header>
              <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                <th class="px-2 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center w-12">م</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">الرتبة</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">الرقم العسكري</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">الاسم</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">محل الخدمة</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">الجهة الواصل منها</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">تاريخ المباشرة</th>
                <th class="px-4 py-3 font-bold text-center">ملاحظات</th>
              </tr>
            </template>
            ...
            <template #body>
              <tr v-for="(row, idx) in filteredReportData" :key="row.index" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-200 dark:border-gray-700">
                ...
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700">{{ row.new_workplace || row.unit || '-' }}</td>
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700">{{ row.arrived_from || '-' }}</td>
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700 font-mono">{{ row.start_date || '-' }}</td>
          ...
          <div class="mt-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 print:hidden">
            ...
            <ul class="list-disc list-inside space-y-1 text-gray-700 dark:text-gray-300 print:text-black">
              <li><strong>محل الخدمة:</strong> محل الخدمة الحالية في الوحدة.</li>
              <li><strong>الجهة الواصل منها:</strong> الوحدة التي كان الفرد يعمل فيها قبل تصديره إلى الوحدة.</li>
              <li><strong>تاريخ المباشرة:</strong> تاريخ وصول الفرد للوحدة ومباشرته للعمل.</li>
              <li>ينزل الشهر اللاحق على الإدارة المصدر إليها.</li>
            </ul>
          </div>
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get('/reports/detailed-reports/audit-movement/', {
      params: { report_id: 'report_18' }
    })
    reportData.value = res.data.data || []
  } catch (error) {
    console.error('Failed to fetch report 18', error)
  } finally {
    loading.value = false
  }
}
...
const exportExcel = () => {
  exportToCSV([], reportData.value, 'MinistryArrivalsReport_Export.csv')
}
</script>

<style scoped>
/* Print styles are now inherited globally from main.css */
</style>
