  ...
  <admin-layout>
    <div class="space-y-6 pb-20">
      <div class="flex flex-wrap items-center justify-between gap-4 print:hidden">
        <div>
          <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">كشف بالقوة غير العاملة نهائياً مرشحين للتقاعد</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">نموذج رقم (١٤)</p>
        </div>
        ...
        <button @click="printReport" class="flex items-center gap-2 rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 transition-colors">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
            طباعة
          </button>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 overflow-hidden printable-area print:overflow-visible print:border-none print:shadow-none print:bg-transparent">
        <div class="p-6 print:p-0">
          <ReportHeader title="كشف بالقوة غير العاملة نهائياً مرشحين للتقاعد" />
          ...
          <ReportTable 
            ...
            emptyDescription="لا يوجد مرشحين للتقاعد."
          ...
          >
            <template #header>
              <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                ...
                <th rowspan="2" class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">نوع الحالة</th>
            ...
            <template #body>
              <tr v-for="(row, idx) in filteredReportData" :key="row.index" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-200 dark:border-gray-700">
                ...
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700">{{ row.status_type || row.reason || row.case_type || '-' }}</td>
                ...
                <td class="px-3 py-3 text-center border-l border-gray-200 dark:border-gray-700">
                  <span v-if="row.is_completed || row.procedures === 'مستكمل'">✔</span>
                </td>
                <td class="px-3 py-3 text-center border-l border-gray-200 dark:border-gray-700">
                  <span v-if="!row.is_completed && row.procedures !== 'مستكمل'">✔</span>
                </td>
          ...
          <div class="mt-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 print:hidden">
            ...
            <ul class="list-disc list-inside space-y-1 text-gray-700 dark:text-gray-300 print:text-black">
              ...
              <li><strong>الإجراءات:</strong> ١- طلب شخصي من الفرد. ٢- أن يكون قد أمضى في الخدمة عشرين عام أو بلغ من العمر خمسون عام. ٣- عدم اللياقة ..... الخ.</li>
              <li>وجود مذكرة من الوزارة بضمة على كشوفات المرشحين للتقاعد.</li>
              <li><strong>نوع الحالة:</strong> بلوغ السن القانوني - عدم لياقة - نهاية المدة ........ الخ.</li>
            </ul>
          </div>
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get('/reports/detailed-reports/perm-inactive/', {
      params: { report_id: 'report_14' }
    })
    reportData.value = res.data.data || []
  } catch (error) {
    console.error('Failed to fetch report 14', error)
  } finally {
    loading.value = false
  }
}
...
const exportExcel = () => {
  exportToCSV([], reportData.value, 'RetirementCandidatesReport_Export.csv')
}
</script>
