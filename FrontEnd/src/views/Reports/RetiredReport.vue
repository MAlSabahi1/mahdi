  ...
  <admin-layout>
    <div class="space-y-6 pb-20">
      <div class="flex flex-wrap items-center justify-between gap-4 print:hidden">
        <div>
          <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">كشف بالقوة غير العاملة متقاعدين</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">نموذج رقم (١٧)</p>
        </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 overflow-hidden printable-area print:overflow-visible print:border-none print:shadow-none print:bg-transparent">
        <div class="p-6 print:p-0">
          <ReportHeader title="كشف بالقوة غير العاملة متقاعدين" />
          ...
          <ReportTable 
            ...
            emptyDescription="لا يوجد متقاعدين مسجلين."
          ...
          >
            <template #header>
              <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                ...
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">تاريخ الميلاد</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">تاريخ الالتحاق</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">رقم القرار</th>
                <th class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">تاريخ القرار</th>
            ...
            <template #body>
              <tr v-for="(row, idx) in filteredReportData" :key="row.index" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-200 dark:border-gray-700">
                ...
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700 font-mono">{{ row.decision_number || '-' }}</td>
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700 font-mono">{{ row.decision_date || '-' }}</td>
          ...
          <div class="mt-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 print:hidden">
            ...
            <ul class="list-disc list-inside space-y-1 text-gray-700 dark:text-gray-300 print:text-black">
              ...
              <li><strong>رقم القرار:</strong> رقم القرار الوزاري بالإحالة للتقاعد.</li>
              <li><strong>تاريخ القرار:</strong> تاريخ القرار الوزاري بالإحالة للتقاعد.</li>
              <li>يرفع لمرة واحدة عند وصول بلاغ القرار وينزلو من كشف الخدمات الشهر اللاحق.</li>
            </ul>
          </div>
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get('/reports/detailed-reports/perm-inactive/', {
      params: { report_id: 'report_17' }
    })
    reportData.value = res.data.data || []
  } catch (error) {
    console.error('Failed to fetch report 17', error)
  } finally {
    loading.value = false
  }
}
...
@media print {
  ...
  /* Force table styles for formal printing */
  :deep(table) {
    width: 100% !important;
