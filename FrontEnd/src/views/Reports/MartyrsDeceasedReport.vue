  ...
  <admin-layout>
    <div class="space-y-6 pb-20">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4 print:hidden">
        <div>
          <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">كشف بالقوة غير العاملة شهداء ووفيات</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">تقرير تفصيلي بحالات الاستشهاد والوفيات</p>
        </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 overflow-hidden printable-area print:overflow-visible print:border-none print:shadow-none print:bg-transparent">
        <div class="p-6 print:p-0">
          ...
          <ReportTable 
            ...
            emptyDescription="لا يوجد شهداء أو وفيات مسجلة."
          ...
          >
            ...
            <template #header>
              <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                ...
                <th colspan="2" class="px-4 py-2 border-b border-l border-gray-200 dark:border-gray-700 font-bold text-center bg-brand-50/50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300">نوع الحالة</th>
              ...
              <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <th class="px-3 py-2 border-l border-gray-200 dark:border-gray-700 text-sm font-semibold text-center">شهيد</th>
                <th class="px-3 py-2 border-l border-gray-200 dark:border-gray-700 text-sm font-semibold text-center">متوفى</th>
                <th class="px-3 py-2 border-l border-gray-200 dark:border-gray-700 text-sm font-semibold text-center">أثناء الواجب</th>
                <th class="px-3 py-2 border-l border-gray-200 dark:border-gray-700 text-sm font-semibold text-center">طبيعية</th>
              </tr>
            </template>
            ...
            <template #body>
              <tr v-for="(row, idx) in filteredReportData" :key="row.index" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-200 dark:border-gray-700">
                ...
                <td class="px-3 py-3 text-center border-l border-gray-200 dark:border-gray-700">
                  <span v-if="row.case_type?.includes('شهيد') || row.case_type?.includes('شهداء')">✔</span>
                </td>
                <td class="px-3 py-3 text-center border-l border-gray-200 dark:border-gray-700">
                  <span v-if="row.case_type?.includes('متوفى') || row.case_type?.includes('وفيات')">✔</span>
                </td>
                
                <!-- تاريخ وقوعها -->
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700">{{ row.death_date || '-' }}</td>
          ...
          <div class="mt-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 print:hidden">
            ...
            <ul class="list-disc list-inside space-y-1 text-gray-700 dark:text-gray-300 print:text-black">
              <li><strong>نوع الحالة:</strong> شهيد أو متوفى بحسب الحالة.</li>
              <li><strong>تاريخ وقوعها:</strong> تاريخ وقوع الاستشهاد أو الوفاة.</li>
              <li><strong>حالة وقوعها:</strong> وقوع الحالة أثناء الواجب أم طبيعية.</li>
              <li><strong>ملاحظات:</strong> أي بيانات أخرى.</li>
            </ul>
          </div>
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get('/reports/detailed-reports/perm-inactive/', {
      params: { report_id: 'report_16' }
    })
    reportData.value = res.data.data || []
  } catch (error) {
    console.error('Failed to fetch report_16', error)
  } finally {
    loading.value = false
  }
}
...
const exportExcel = () => {
  exportToCSV([], reportData.value, 'MartyrsDeceasedReport_Export.csv')
}
</script>
