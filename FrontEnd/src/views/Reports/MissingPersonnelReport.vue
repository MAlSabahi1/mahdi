  ...
  <admin-layout>
    <div class="space-y-6 pb-20">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4 print:hidden">
        <div>
          <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">كشف بالقوة غير العاملة مؤقتاً مفقودين</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">نموذج رقم (١١)</p>
        </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 overflow-hidden printable-area print:border-none print:shadow-none print:bg-transparent">
        <div class="p-6 print:p-0">
          <ReportHeader title="كشف بالقوة غير العاملة مؤقتاً مفقودين" />
          ...
          <ReportTable 
            ...
            emptyDescription="لا يوجد مفقودين مسجلين."
          ...
          >
            ...
            <template #header>
              <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                ...
                <th rowspan="2" class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">محل الخدمة</th>
                <th rowspan="2" class="px-4 py-3 border-l border-gray-200 dark:border-gray-700 font-bold text-center">تاريخ الفقدان</th>
            ...
            <template #body>
              <tr v-for="(row, idx) in filteredReportData" :key="row.index" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-200 dark:border-gray-700">
                ...
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700">{{ row.unit || row.status || '-' }}</td>
                <td class="px-4 py-3 text-center border-l border-gray-200 dark:border-gray-700">{{ row.missing_date || row.incident_date || '-' }}</td>
                
                <!-- الإجراءات -->
                <td class="px-3 py-3 text-center border-l border-gray-200 dark:border-gray-700">
                  <span v-if="row.is_completed || row.court_order === 'مستكمل'">✔</span>
                </td>
                <td class="px-3 py-3 text-center border-l border-gray-200 dark:border-gray-700">
                  <span v-if="!row.is_completed && row.court_order !== 'مستكمل'">✔</span>
                </td>
          ...
          <div class="mt-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 print:border-none print:bg-transparent">
            ...
            <ul class="list-disc list-inside space-y-1 text-gray-700 dark:text-gray-300 print:text-black">
              <li><strong>تاريخ الفقدان:</strong> تاريخ وقوع الفقدان.</li>
              <li><strong>الإجراءات:</strong> ١- حكم شرعي بالفقدان. ٢- وكالة شرعية. ٣- حكم انحصار ورثة.</li>
              <li><strong>مستكمل:</strong> أوصل الوثائق الموضحة.</li>
              <li><strong>غير مستكمل:</strong> لم يوصل الوثائق.</li>
            </ul>
          </div>
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get('/reports/detailed-reports/temp-inactive/', {
      params: { report_id: 'report_11' }
    })
    reportData.value = res.data.data || []
  } catch (error) {
    console.error('Failed to fetch report 11', error)
  } finally {
    loading.value = false
  }
}
